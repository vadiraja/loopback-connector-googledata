...
describe('When set Google Data options', function () {
    var testConnector;
    ...
    it('should return a valid URL for elastic client', function (done) {
     ...
     it('should return a valid Google Data Connector', function (done) {
         
        testConnector.connectToGoogleDataStore(testConnector.settings,testConnector.dataSource);
        var clientURL = testConnector.ds;
        expect(clientURL).not.to.be.null;
        expect(clientURL.projectId).to.be.a('string').to.equal('in-forums');
        done();
    });
});